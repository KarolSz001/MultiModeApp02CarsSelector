...
public class CarValidatorImpl extends AbstractValidator<Car> {
    ...
    public Map<String, String> validate(Car car) {
        ...
        if (!isModelCorrect(car.getModel())) {
            errors.put("Error nr 1", "model name only work with Letters");
        }
        if (!allComponentsCorrect(car.getCarBody().getComponents())) {
            errors.put("Error nr 2", "components name only work with Letters");
        }
        if (!isPriceCorrect(car.getPrice())) {
            errors.put("Error nr 3", "price is negative number");
        }
        if (!isMileageCorrect(car.getMileage())) {
            errors.put("Error nr 4", "millage is out of range");
        }
        return errors;
    }
